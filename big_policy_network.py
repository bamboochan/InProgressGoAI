...
tar = tarfile.open("pro1.tar.gz", 'r:gz')
saver = tf.train.Saver()
saver.restore(sess, 'big_policy_network4.ckpt')
with open('filenames_baduk.txt', 'r') as filenames:
    for num, line in enumerate(filenames):
#        if num < 5000:
#            continue
#        print(line)
        bad, batch_in, batch_out = inp.getdata(tar, "./pro1/" + line[:-1])
...
#            print(res_flat.eval(feed_dict={x: batch_in, y1: batch_out, keep_prob: 1.0}))
            ...
            if num % 5000 == 4999:
                save_path = saver.save(sess, 'big_policy_network5.ckpt')
    save_path = saver.save(sess, 'big_policy_network_final5.ckpt')
